...
function extractInvoiceItems(text) {
  ...
  for (const line of lines) {
    ...
    if (qtyPriceMatch) {
      ...
      quantity = parseInt(qtyPriceMatch, 10);
...
/**
 * Base document parser runner engine.
 * 
...
function parseDocument(rawText) {
  ...
  if (normalizedText.includes('invoice') || normalizedText.includes('bill to') || normalizedText.includes('amount due')) {
    ...
    // Clean headers and capture group indices explicitly to prevent the text duplication bug
...
function buildAiPrompt(rawText) {
  return `Analyze extraction context parameters:\n${rawText}`;
}
